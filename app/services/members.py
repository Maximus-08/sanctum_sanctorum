...
def get_member_stats(db: Session, member_id: int, now: datetime) -> MemberStats:
    ...
    orders_paid, total_spent_cents = db.execute(paid_orders).one()

    loan_totals = select(
        func.count(Loan.id).filter(Loan.returned_at.is_(None)),
        # Strictly past due: a loan sitting exactly on due_at is not yet overdue.
        func.count(Loan.id).filter(Loan.returned_at.is_(None), Loan.due_at < now),
        func.coalesce(func.sum(Loan.late_fee_cents).filter(Loan.returned_at.is_not(None)), 0),
    ).where(Loan.member_id == member_id)
    active_loans, overdue_loans, late_fees_cents = db.execute(loan_totals).one()
